...
```{r, eval = FALSE, echo = TRUE}
#extract one
mean(d2$data[[1]]$residential) # I could not find d2
...
try1 <- function(df){
  ...
  df_num <- as.numeric(df) # # AGI: maybe you want to add a "if (condition) {}" instead of this so you make sure that you have numeric values in the df. Otherwise you would just be coercing values that may not be numeric, for instance a dataframe that has mostly categorical variables like gss_cat.
